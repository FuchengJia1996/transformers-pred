    ...
    def forward(self, hidden_states: torch.FloatTensor) -> torch.FloatTensor:
        if global_weight_preditor is not None and is_sparse_infer():
            ...
            if global_tensor_saver is not None:
                global_tensor_saver.save(up_states, self.layer_idx, "downi")
            return self.down_proj(up_states)
...
class Phi3SparseAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class Phi3DecoderLayer(nn.Module):
    def __init__(self, config: Phi3Config, layer_idx: int):
        ...
        self.self_attn = Phi3SparseAttention(config, layer_idx=layer_idx)

        #self.mlp = Phi3MLP(config)
        self.mlp = Phi3SparseMLP(config, layer_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if global_tensor_saver is not None:
            global_tensor_saver.save(attn_outputs, self.layer_idx, "ao")
            global_tensor_saver.save(residual, self.layer_idx, "aor")

        hidden_states = residual + self.resid_attn_dropout(attn_outputs)
        if global_tensor_saver is not None:
            global_tensor_saver.save(hidden_states, self.layer_idx, "aorr")
        ...
        if global_tensor_saver is not None:
            global_tensor_saver.save(hidden_states, self.layer_idx, "morr")
